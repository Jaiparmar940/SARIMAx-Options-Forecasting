...
def process():
    ...
    for i in sList:
        print('retrieving max data for ' + i.getName() + ' via yahoo finance.')
        yFinance.getPrices(i, "max")
        print( i.getName() + ' data collection complete.')
        print( '------------------------------------------------------------')
    ...
    while (index < len(sList)):
        ...
        processes.append( Process(target = runLstm , args =( sList[index], days, lookback )))
        index = index + 1
